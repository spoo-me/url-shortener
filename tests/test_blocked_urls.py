...
def test_url_blocked(mocker):
    ...
    mocker.patch(
        "utils.blocked_urls_collection.find", return_value=[{"_id": "example.com"}]
    )
...
def test_url_partially_blocked(mocker):
    # Mock the database call to return a list with a blocked URL pattern
    mocker.patch(
        "utils.blocked_urls_collection.find", return_value=[{"_id": "example"}]
    )
...
def test_url_with_no_match(mocker):
    ...
    mocker.patch(
        "utils.blocked_urls_collection.find", return_value=[{"_id": "blocked.com"}]
    )
